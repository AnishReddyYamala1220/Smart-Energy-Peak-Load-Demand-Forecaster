...
def create_features(df):
    ...
    return data.dropna()
def train_and_forecast(df, n_estimators=500, learning_rate=0.03, max_depth=6, contamination=0.03):
    """Trains XGBoost forecasting model & Isolation Forest anomaly detector."""
    ...
    # 1. Train XGBoost Forecast Model
    ...
    # Features used: actual load vs historical expectations
    iso = IsolationForest(
        contamination=contamination, 
        random_state=42,
        n_estimators=100
    )
    # Train anomaly model on training data features + target
    train_anomaly_features = train[feature_cols + [target_col]]
    test_anomaly_features = test[feature_cols + [target_col]]
    iso.fit(train_anomaly_features)
    # Predict anomalies (-1 = anomaly, 1 = normal)
    test['Anomaly_Flag'] = iso.predict(test_anomaly_features)
    test['Is_Anomaly'] = test['Anomaly_Flag'] == -1
    # Evaluate Metrics
